...
export default function ServicesShowCase() {
  ...
  return (
    <section className="pt-15 pb-20 lg:py-20 px-8">
      <div className="max-w-277.75 mx-auto grid lg:grid-cols-2 gap-x-7.5 gap-y-6">
        {services.map((service, id) => (
          ...
          >
            ...
            />
            <div
              className={`absolute inset-0 ${service.backgroundColor} flex flex-col items-center justify-center gap-6`}
            >
              <h2 className="text-[40px]/12 text-white">{service.title}</h2>
              <div className="flex items-center gap-3">
                <span className="text-white uppercase text-[15px] tracking-[5px]">
                  View Project{" "}
                </span>
                <MdOutlineKeyboardArrowRight className="text-xl text-Peach" />
              </div>
            </div>
          </Link>
        ))}
